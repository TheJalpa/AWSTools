...
# Lastly we are skipping input and the plan, this will just do a terraform apply.
    try:
        ...
        #On the line below we can over-write any existing vars with ones we want by simply calling in our custom vars file "pyvars.py"
        #This is imported at the top of our script.
        #This is nice for when you want to set custom variables without having to dig through a giant complex vars.tf file, or just want devs to pass them
        return_code, stdout, stderr = tf.apply(input=False, skip_plan=True, capture_output=False, var={'ec2name': ec2name, 'ec2service': ec2service, 'ec2department': ec2department})
